                ...
                <x-project-form action="{{ route('projects.update',$project->id) }}" method="POST" class="col-md-4">
                        ...
                        <select class="form-select" name="owner_id">
                            ...
                            @foreach ($users as $user)
                                <option value="{{$user->id}}"
                                        @if($project->owner_id == $user->id) selected @endif>{{$user->username}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="assignee_id">
                            ...
                            @foreach ($users as $user)
                                <option value="{{$user->id}}"
                                        @if($project->assignee_id == $user->id) selected @endif>{{$user->username}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        <input class="form-check-input" type="checkbox" name="is_active" @if($project->is_active) checked @endif>
